...
def affine_forward(x, w, b):
    ...
    #或x1 = x.reshape(x.shape[0],-1)
    out = np.dot(x1,w) + b.reshape(1,-1) #(N,M)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    cache = (x, w, b)
...
def batchnorm_forward(x, gamma, beta, bn_param):
    """
    function:对输入数据进行预处理(归一化、缩放、平移);先将input转为标准正态分布，然后进行仿射变换
    输入： x, gamma, beta, 以及其他参数的字典
    输出： 处理后的输入数据，及一些参数的缓存
    -------------------------------------------------
    ...
    if mode == 'train':
        ...
        # variables.                                                          #
        #######################################################################
        sample_mean = np.mean(x,axis = 0)#np.mean()求均值
        sample_var = np.var(x,axis = 0) # np.var() 求方差
        x_hat = (x - sample_mean)/ (np.sqrt(sample_var + eps))
        out = gamma * x_hat + beta
        cache = (x,sample_mean,sample_var,x_hat,eps,gamma,beta)
        #然后计算running mean 和 running_var，以便test时用
        running_mean = momentum * running_mean + (1 - momentum) * sample_mean
        running_var = momentum * running_var + (1 - momentum) * sample_var
        #######################################################################
        #                           END OF YOUR CODE                          #
...
def dropout_forward(x, dropout_param):
    ...
    if mode == 'train':
        ...
        #由于dropout，out的整体分布的期望值下降，因此，在test时，x应乘以prob，以保持分布的统一
        #但也可直接在train阶段用mask除以prob，保持分布的统一
        out = mask * x
        #######################################################################
        #                           END OF YOUR CODE                          #
        #######################################################################
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    N,C,H,W = x.shape       
    F,_,HH,WW = w.shape
    #np.pad(array,tuple,'constant'),tuple内几个元组即对几个维度操作，(0,0)表示不填充，(1,1)表示填充上下或左右各一个元素
    ...
    for i in range(H_out):
        for j in range(W_out):
            x_mask = x_pad[:,:,i*conv_param['stride']:i*conv_param['stride']+HH, j*conv_param['stride']:j*conv_param['stride']+WW]
            for k in range(F):
                out[:,k,i,j] = np.sum(x_mask * w[k,:,:,:],axis=(1,2,3))#axis表示(0,1,2,3)中只对后三维求和
    out = out + b[None,:,None,None]  #None相当于增加一个维度
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    cache = (x, w, b, conv_param)
